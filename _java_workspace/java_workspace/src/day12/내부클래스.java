...
public class 내부클래스 {
	...
	public static void main(String[] args) {
		 ...
		 * 내부클래스를 사용할 일이 없을경우
		 * - 중첩 클래스
		 * - 인스턴스 내부클래스, 정적(static) 내부클래스, 지역(local) 내부 클래스, 
		 * - 익명(anonymous) 내부클래스 -> 익명클래스를 사용하기 위해 내부클래스를 사용.
		 * 
		 * - 내부적으로 활용할 목적으로 만드는 클래스이기 떄문에 private으로 선언하는 것을 권장
		 * - 내부 클래스의 생성시기: 외부클래스가 생성된 후 생성됨.(static은 다름)
		 * - private이 아닌 내부 클래스는 다른 외부클래스에서 생성할 수 있다.
		 * */
...
//		static void sTest() {} error
	...
	static class InStaticClass {
		...
		static void sTest() { //정적클래스의 정적메서드
			//System.out.println("Outclass Num" + num + "(외부클래스의 인스턴스 변수)");
			System.out.println("Inclass sInNum" + sInNum + "(내부클래스의 인스턴스 변수)");
			System.out.println("Outclass sNum" + sNum + "(외부클래스의 static 변수)");
			//System.out.println("Inclass inNum" + inNum + "(내부클래스의 인스턴스 변수)");
