...
class Boom {
	...
	public void printMenu() {
		...
		System.out.println("▶ 같이 게임을 진행할 플레이어 3명을 추가합니다(본인포함)");
			...
			while (true) {
				...
				for (int i = 1; i < SIZE; i++) {
					if (nowplayer.equals(player.get(i))) {
						count++;
						System.out.println("[" + player.get(i) + "]" + "님에게 💣 넘어갔습니다111.");
				...
				while (true) {
					...
					if (player.get(playerRandom).equals(nowplayer)) {
						...
						count++;
						System.out.println("[" + player.get(playerRandom) + "]" + "님에게 💣 넘어갔습니다.222");
				...
				if (!nowplayer.equals(player.get(0)) || bomb != count) {
					while (true) {
						...
						if (player.get(playerRandom).equals(nowplayer)) {
							...
							count++;
							System.out.println("[" + player.get(playerRandom) + "]" + "님에게 💣 넘어갔습니다333");
							System.out.println("[" + player.get(playerRandom) + "]" + "님이 💁🏻=͟͟͞͞💣 넘길 차례입니다.");
							System.out.println();
							nowplayer = player.get(playerRandom);
							if (bomb <= count) {
