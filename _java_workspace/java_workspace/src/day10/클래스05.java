...
public class 클래스05 {
	...
	public static void main(String[] args) {
		 ...
		 * static이 안붙은 멤버변수/메서드들은 객체(인스턴스) 멤버변수/메서드라고한다.
		 * 
		 ...
		 * -객체 멤버 이미 클래스가 생성된 후에 생성 / 클래스의 멤버 메서드를 객체에서 사용될 수 있다.
		 * -클래스 멤버 변수는 클래스/객체 멤버에서 모두 사용할 수 있다.
		 * -클래스 멤버 메서드는 클래스/객체 멤버에서 모두 사용할 수 있다.
		 * -객체 멤버 변수는 클래스 멤버 메서드에서 사용 할 수 없다.
		 * -객체 메서드는 객체멤버 메서드에서만 사용할 수 있다.
		 * */
		TV t = new TV();
		//개체 멤버 메서드 접근방식: 객체생성후 객체명.메서드명();
		t.printPower();
		System.out.println(TV.brand);
		
		//클래스 멤버 메서드 접근방식: 클래스명.메서드();
		TV.printBrand();
